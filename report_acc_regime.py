...
def init_acc_regime(dataname):
    ...
    elif 'RPV' in dataname:
        return init_acc_regime_rpv()
    else:
...
def update_acc_regime(dataname, acc_regime, model_output, target, structure_encoded, data_file):
    ...
    elif 'RVP' in dataname:
        update_acc_regime_rpv(acc_regime, model_output, target, data_file)
    else:
...
def update_acc_regime_rpv(acc_regime, model_output, target, data_file):
    acc_one = model_output.data.max(1)[1] == target
    for i in range(model_output.shape[0]):
        acc_regime[0][0] += acc_one[i].item()
        acc_regime[0][1] += 1
