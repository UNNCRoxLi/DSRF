...
class PredictiveReasoningBlock(nn.Module):
    ...
    def __init__(
        ...
        self.convg = nn.Sequential(ConvNormAct(9, 36, 3, 1), ConvNormAct(36, 9, 3, 1))
        self.GM = nn.Sequential(nn.LayerNorm(in_planes), nn.Linear(in_planes, in_planes*2))
        self.m = nn.Linear(25, 25)
    ...
    def forward(self, x):
        ...
        b, c, t, l = x.size()
        g, x = self.GM(x.permute(0,2,3,1)).chunk(2,dim=-1)
        g = self.convg(g.contiguous()).permute(0,3,1,2)
...
class NewHGRC(nn.Module):
    ...
    def forward(self, x, train=False):
        ...
        swin_output = self.dim_reducer2(swin_output[2].view(-1, 256,5,5))

        for l in range(4): 
            x = getattr(self, "res"+str(l))(x)
            ###################################
             # [torch.Size([16, 32, 40, 40]), 
                                  #  torch.Size([16, 64, 20, 20]), 
                                  #  torch.Size([16, 128, 10, 10]), 
                                  #  torch.Size([16, 256, 5, 5])]
        x = self.channel_reducer(x+swin_output)
        _, c, h, w = x.size()
